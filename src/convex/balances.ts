...
function calculateExpenseSplit(
	...
	switch (expense.splitType) {
		...
		case 'exact':
			const exactDetail = expense.splitDetails?.find((d) => d.memberId === memberId);
			return exactDetail?.value ?? 0;

		case 'percentage':
			const percentDetail = expense.splitDetails?.find((d) => d.memberId === memberId);
...
export const getOptimizedSettlements = query({
	...
	handler: async (ctx, args) => {
		...
		const debtors = netBalances.filter((m) => m.balance < 0).sort((a, b) => a.balance - b.balance);
...
export const getMemberExpenseBreakdown = query({
	...
	handler: async (ctx, args) => {
		...
		const owedExpenses = expenses.filter((e) => e.splitAmong.includes(args.memberId));

		const breakdown = {
			memberName: member.name,
			totalPaid: paidExpenses.reduce((sum, e) => sum + e.amount, 0),
			totalOwed: owedExpenses.reduce((sum, e) => sum + calculateExpenseSplit(e, args.memberId), 0),
