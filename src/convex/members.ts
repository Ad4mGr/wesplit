...
export const remove = mutation({
	...
	handler: async (ctx, args) => {
		...
		if (expensesAsPayer) {
			throw new Error('Cannot delete member with expense history. Deactivate instead.');
		}
		...
		if (isInSplit) {
			throw new Error('Cannot delete member included in expense splits. Deactivate instead.');
		}
