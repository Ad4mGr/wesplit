...
export const createEqual = mutation({
	...
	handler: async (ctx, args) => {
		...
		if (args.splitAmong.length === 0) throw new Error('Must split among at least one member');
		...
		for (const memberId of args.splitAmong) {
			if (memberId !== args.paidBy) {
				const member = await ctx.db.get(memberId);
				if (member) {
					const share = args.amount / args.splitAmong.length;
					await ctx.db.insert('notifications', {
						memberId,
						groupId: args.groupId,
						type: 'expense_added',
						title: `New expense in ${group.name}`,
						message: `${payer?.name ?? 'Someone'} added "${args.description}" — your share: $${share.toFixed(2)}`,
						fromMemberId: args.paidBy,
						expenseId,
						createdAt: Date.now(),
						readAt: undefined
					});
...
export const createExact = mutation({
	...
	handler: async (ctx, args) => {
		...
		if (args.amount <= 0) throw new Error('Amount must be positive');
		if (args.splitDetails.length === 0) throw new Error('Must have at least one split detail');

		// Validate exact amounts sum to total
		const totalSplit = args.splitDetails.reduce((sum, detail) => sum + detail.value, 0);
		...
		// Notify all members except the payer
		const payer = await ctx.db.get(args.paidBy);
		for (const detail of args.splitDetails) {
			if (detail.memberId !== args.paidBy) {
				const member = await ctx.db.get(detail.memberId);
				if (member) {
					await ctx.db.insert('notifications', {
						...
						type: 'expense_added',
						title: `New expense in ${group.name}`,
						message: `${payer?.name ?? 'Someone'} added "${args.description}" — your share: $${detail.value.toFixed(2)}`,
						fromMemberId: args.paidBy,
						expenseId,
						createdAt: Date.now(),
						readAt: undefined
...
export const createPercentage = mutation({
	...
	handler: async (ctx, args) => {
		...
		const totalPercentage = args.splitDetails.reduce((sum, detail) => sum + detail.value, 0);
		...
		for (const detail of args.splitDetails) {
			if (detail.memberId !== args.paidBy) {
				const member = await ctx.db.get(detail.memberId);
				if (member) {
					const share = (args.amount * detail.value) / 100;
					await ctx.db.insert('notifications', {
						memberId: detail.memberId,
						groupId: args.groupId,
						type: 'expense_added',
						title: `New expense in ${group.name}`,
						message: `${payer?.name ?? 'Someone'} added "${args.description}" — your share: $${share.toFixed(2)} (${detail.value}%)`,
						fromMemberId: args.paidBy,
						expenseId,
						createdAt: Date.now(),
						readAt: undefined
...
export const update = mutation({
	args: {
		...
		splitType: v.optional(v.union(v.literal('equal'), v.literal('exact'), v.literal('percentage'))),
	...
	handler: async (ctx, args) => {
		...
		if (updates.splitDetails && updates.amount) {
			const totalSplit = updates.splitDetails.reduce((sum, detail) => sum + detail.value, 0);
			if (expense.splitType === 'exact' || updates.splitType === 'exact') {
				if (Math.abs(totalSplit - updates.amount) > 0.01) {
					throw new Error('Split amounts must equal total expense amount');
				}
			}
			if (expense.splitType === 'percentage' || updates.splitType === 'percentage') {
